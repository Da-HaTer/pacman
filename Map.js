...
class Pathtile{
    constructor({position,state=1})//state: 0 empty, 1 has pelet 2: has super pelet etc
    {
...
var Map = function(numCols, numRows, tiles) {
    ...
    this.routes=new Array(this.numRows)
    this.resetCurrent();
    this.parseWalls();
    this.init_tiles();
};
Map.prototype.init_tiles= function(){
    for (let x=0; x<this.numRows; x++) {
        this.routes[x]=new Array(this.numCols).fill(-1)
        for (let y=0; y<this.numCols; y++) {
            if (this.isFloorTile(y,x)){
                this.routes[x][y]=1
            }
        }
    }
...
Map.prototype.drawmap = function(ctx,left,top,print) {
...
Map.prototype.draw_pelets=function(ctx,left,top,print){
    ctx.save();
    ctx.translate(0.5,0.5); // pixel perfect lines?
...
Map.prototype.draw_grid=function(ctx,left,top,print){
    ctx.save();
    ctx.translate(0.5,0.5); // pixel perfect lines?
    ...
    ctx.beginPath();
    for (let y=4; y<this.numRows-2; y++) {
        ctx.moveTo(0,y*tileSize);
        ctx.lineTo(this.widthPixels,y*tileSize);
    }
    for (let x=0; x<this.numCols+1; x++) {
        ctx.moveTo(x*tileSize,4*tileSize);
        ctx.lineTo(x*tileSize,this.heightPixels-tileSize-2*tileSize);
    }
    ctx.stroke();
...
Map.prototype.drawPath = function(ctx,left,top) {
    ...
    ctx.beginPath();
    for (let y=0; y<this.numRows-1; y++) {
        for (let x=0; x<this.numCols-1; x++) {
